...
def extract_showtimes(html):
    ...
    GENRE_WORDS = {
        ...
        'teatro','nation','live','mins','running','sci-fi','horror','crime',
        'historical','war','sport','western','noir','suspense','supernatural',
        'psychological','political','satire','parody','documentary'
    }
    SKIP_LINES = {'showtimes','check our socials','about us','stay connected','tiktok'}
    # Also skip any line that looks like a genre combination (e.g. "Comedy, Drama")
    GENRE_LINE_PAT = re.compile(
        r'^(?:Drama|Comedy|Music|Family|Adventure|Animation|Thriller|Romance|'
        r'Documentary|Biography|Musical|Theatre|Play|Opera|Fantasy|Biopic|'
        r'Action|Mystery|Science Fiction|Romantic|Sci-Fi|Horror|Crime|'
        r'Nation|Live|Suspense|Psychological|Political|Satire|Teatro|'
        r'Science|Fiction|Nation Theatre)(?:\s*,\s*(?:Drama|Comedy|Music|Family|'
        r'Adventure|Animation|Thriller|Romance|Documentary|Biography|Musical|'
        r'Theatre|Play|Opera|Fantasy|Biopic|Action|Mystery|Science Fiction|'
        r'Romantic|Sci-Fi|Horror|Crime|Nation|Live|Suspense|Psychological|'
        r'Political|Science|Fiction|Nation Theatre Live))*
    ...
    while i < len(lines):
        ...
        # Skip pure genre lines (e.g. "Comedy, Drama" or "Play, Theatre, Drama")
        words = set(re.sub(r'[^a-z, ]', '', line.lower()).split())
        words.discard('')
        if words and words.issubset(GENRE_WORDS):
            i += 1
            continue
        if GENRE_LINE_PAT.match(line):
            i += 1
            continue
